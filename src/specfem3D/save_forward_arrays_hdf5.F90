...
subroutine save_intermediate_forward_arrays_hdf5()
  ...
  if (myrank == 0) then
    ...
    call h5_create_dataset_gen('displ_outer_core', (/NDIM, sum(offset_nglob_oc)/), 2, CUSTOM_REAL)
    call h5_create_dataset_gen('veloc_outer_core', (/NDIM, sum(offset_nglob_oc)/), 2, CUSTOM_REAL)
    call h5_create_dataset_gen('accel_outer_core', (/NDIM, sum(offset_nglob_oc)/), 2, CUSTOM_REAL)
    ...
    call h5_create_dataset_gen('epsilondev_xx_crust_mantle', (/NGLLX, NGLLY, NGLLZ, sum(offset_nglob_mc_str_or_att)/), 4, CUSTOM_REAL)
    call h5_create_dataset_gen('epsilondev_yy_crust_mantle', (/NGLLX, NGLLY, NGLLZ, sum(offset_nglob_mc_str_or_att)/), 4, CUSTOM_REAL)
    call h5_create_dataset_gen('epsilondev_xy_crust_mantle', (/NGLLX, NGLLY, NGLLZ, sum(offset_nglob_mc_str_or_att)/), 4, CUSTOM_REAL)
    call h5_create_dataset_gen('epsilondev_xz_crust_mantle', (/NGLLX, NGLLY, NGLLZ, sum(offset_nglob_mc_str_or_att)/), 4, CUSTOM_REAL)
    call h5_create_dataset_gen('epsilondev_yz_crust_mantle', (/NGLLX, NGLLY, NGLLZ, sum(offset_nglob_mc_str_or_att)/), 4, CUSTOM_REAL)
  ...
  ! write datasets
  call h5_write_dataset_collect_hyperslab('displ_crust_mantle', displ_crust_mantle, (/0, sum(offset_nglob_cm(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('veloc_crust_mantle', veloc_crust_mantle, (/0, sum(offset_nglob_cm(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('accel_crust_mantle', accel_crust_mantle, (/0, sum(offset_nglob_cm(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('displ_outer_core', displ_outer_core, (/0, sum(offset_nglob_oc(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('veloc_outer_core', veloc_outer_core, (/0, sum(offset_nglob_oc(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('accel_outer_core', accel_outer_core, (/0, sum(offset_nglob_oc(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('displ_inner_core', displ_inner_core, (/0, sum(offset_nglob_ic(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('veloc_inner_core', veloc_inner_core, (/0, sum(offset_nglob_ic(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('accel_inner_core', accel_inner_core, (/0, sum(offset_nglob_ic(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('epsilondev_xx_crust_mantle', epsilondev_xx_crust_mantle, (/0, 0, 0, sum(offset_nglob_mc_str_or_att(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('epsilondev_yy_crust_mantle', epsilondev_yy_crust_mantle, (/0, 0, 0, sum(offset_nglob_mc_str_or_att(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('epsilondev_xy_crust_mantle', epsilondev_xy_crust_mantle, (/0, 0, 0, sum(offset_nglob_mc_str_or_att(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('epsilondev_xz_crust_mantle', epsilondev_xz_crust_mantle, (/0, 0, 0, sum(offset_nglob_mc_str_or_att(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('epsilondev_yz_crust_mantle', epsilondev_yz_crust_mantle, (/0, 0, 0, sum(offset_nglob_mc_str_or_att(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('epsilondev_xx_inner_core', epsilondev_xx_inner_core, (/0, 0, 0, sum(offset_nglob_ic_str_or_att(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('epsilondev_yy_inner_core', epsilondev_yy_inner_core, (/0, 0, 0, sum(offset_nglob_ic_str_or_att(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('epsilondev_xy_inner_core', epsilondev_xy_inner_core, (/0, 0, 0, sum(offset_nglob_ic_str_or_att(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('epsilondev_xz_inner_core', epsilondev_xz_inner_core, (/0, 0, 0, sum(offset_nglob_ic_str_or_att(0:myrank-1))/), if_col)
  call h5_write_dataset_collect_hyperslab('epsilondev_yz_inner_core', epsilondev_yz_inner_core, (/0, 0, 0, sum(offset_nglob_ic_str_or_att(0:myrank-1))/), if_col)

  if (ROTATION_VAL) then
    call h5_write_dataset_collect_hyperslab('A_array_rotation', A_array_rotation, (/0, 0, 0, sum(offset_nspec_oc_rot(0:myrank-1))/), if_col)
    call h5_write_dataset_collect_hyperslab('B_array_rotation', B_array_rotation, (/0, 0, 0, sum(offset_nspec_oc_rot(0:myrank-1))/), if_col)
  endif

  if (ATTENUATION_VAL) then
    call h5_write_dataset_collect_hyperslab('R_xx_crust_mantle', R_xx_crust_mantle, (/0, 0, 0, 0, sum(offset_nspec_cm_att(0:myrank-1))/), if_col)
    call h5_write_dataset_collect_hyperslab('R_yy_crust_mantle', R_yy_crust_mantle, (/0, 0, 0, 0, sum(offset_nspec_cm_att(0:myrank-1))/), if_col)
    call h5_write_dataset_collect_hyperslab('R_xy_crust_mantle', R_xy_crust_mantle, (/0, 0, 0, 0, sum(offset_nspec_cm_att(0:myrank-1))/), if_col)
    call h5_write_dataset_collect_hyperslab('R_xz_crust_mantle', R_xz_crust_mantle, (/0, 0, 0, 0, sum(offset_nspec_cm_att(0:myrank-1))/), if_col)
    call h5_write_dataset_collect_hyperslab('R_yz_crust_mantle', R_yz_crust_mantle, (/0, 0, 0, 0, sum(offset_nspec_cm_att(0:myrank-1))/), if_col)
    call h5_write_dataset_collect_hyperslab('R_xx_inner_core', R_xx_inner_core, (/0, 0, 0, 0, sum(offset_nspec_ic_att(0:myrank-1))/), if_col)
    call h5_write_dataset_collect_hyperslab('R_yy_inner_core', R_yy_inner_core, (/0, 0, 0, 0, sum(offset_nspec_ic_att(0:myrank-1))/), if_col)
    call h5_write_dataset_collect_hyperslab('R_xy_inner_core', R_xy_inner_core, (/0, 0, 0, 0, sum(offset_nspec_ic_att(0:myrank-1))/), if_col)
    call h5_write_dataset_collect_hyperslab('R_xz_inner_core', R_xz_inner_core, (/0, 0, 0, 0, sum(offset_nspec_ic_att(0:myrank-1))/), if_col)
    call h5_write_dataset_collect_hyperslab('R_yz_inner_core', R_yz_inner_core, (/0, 0, 0, 0, sum(offset_nspec_ic_att(0:myrank-1))/), if_col)
  endif ! ATTENUATION_VAL

  if (FULL_GRAVITY_VAL) then
    call h5_write_dataset_collect_hyperslab('neq', (/neq/), (/myrank/), if_col)
    call h5_write_dataset_collect_hyperslab('neq1', (/neq1/), (/myrank/), if_col)
    call h5_write_dataset_collect_hyperslab('pgrav1', pgrav1, (/sum(offset_pgrav1(0:myrank-1))/), if_col)
  endif ! FULL_GRAVITY_VAL
...
subroutine save_forward_model_at_shifted_frequency_hdf5(factor_scale_relaxed_crust_mantle,factor_scale_relaxed_inner_core)
  ...
  if (NSPEC_CRUST_MANTLE > 0) then
    ...
    if (TRANSVERSE_ISOTROPY) then
      ...
      temp_store(:,:,:,:) = sqrt((kappavstore_crust_mantle(:,:,:,:) &
                            ...
                            * scaleval1
      call h5_write_dataset_collect_hyperslab_in_group('vpv_shifted', temp_store, (/0, 0, 0, sum(offset_nspec_cm(0:myrank-1))/), if_col)
      ! vph
      temp_store(:,:,:,:) = sqrt((kappahstore_crust_mantle(:,:,:,:) &
                            + FOUR_THIRDS * muh_shifted(:,:,:,:))/rhostore_crust_mantle(:,:,:,:)) &
                            * scaleval1
      call h5_write_dataset_collect_hyperslab_in_group('vph_shifted', temp_store, (/0, 0, 0, sum(offset_nspec_cm(0:myrank-1))/), if_col)
      ! vsv
      temp_store(:,:,:,:) = sqrt( muv_shifted(:,:,:,:)/rhostore_crust_mantle(:,:,:,:) )*scaleval1
      call h5_write_dataset_collect_hyperslab_in_group('vsv_shifted', temp_store, (/0, 0, 0, sum(offset_nspec_cm(0:myrank-1))/), if_col)
      ! vsh
      temp_store(:,:,:,:) = sqrt( muh_shifted(:,:,:,:)/rhostore_crust_mantle(:,:,:,:) )*scaleval1
      call h5_write_dataset_collect_hyperslab_in_group('vsh_shifted', temp_store, (/0, 0, 0, sum(offset_nspec_cm(0:myrank-1))/), if_col)
    else ! isotropic
      ! vp
      temp_store(:,:,:,:) = sqrt((kappavstore_crust_mantle(:,:,:,:) &
                            + FOUR_THIRDS * muv_shifted(:,:,:,:))/rhostore_crust_mantle(:,:,:,:)) &
                            * scaleval1
      call h5_write_dataset_collect_hyperslab_in_group('vp_shifted', temp_store, (/0, 0, 0, sum(offset_nspec_cm(0:myrank-1))/), if_col)
      ! vs
      temp_store(:,:,:,:) = sqrt( muv_shifted(:,:,:,:)/rhostore_crust_mantle(:,:,:,:) )*scaleval1
      call h5_write_dataset_collect_hyperslab_in_group('vs_shifted', temp_store, (/0, 0, 0, sum(offset_nspec_cm(0:myrank-1))/), if_col)
  ...
  if (NSPEC_INNER_CORE > 0) then
    ...
    if (ANISOTROPIC_INNER_CORE_VAL) then
      ...
      temp_store(:,:,:,:) = sqrt((kappavstore_inner_core(:,:,:,:) &
                            ...
                            * scaleval1
      call h5_write_dataset_collect_hyperslab_in_group('vp_shifted', temp_store, (/0, 0, 0, sum(offset_nspec_ic(0:myrank-1))/), if_col)
      ! vs
      temp_store(:,:,:,:) = sqrt( muv_shifted(:,:,:,:)/rhostore_inner_core(:,:,:,:) )*scaleval1
      call h5_write_dataset_collect_hyperslab_in_group('vs_shifted', temp_store, (/0, 0, 0, sum(offset_nspec_ic(0:myrank-1))/), if_col)
    endif
  ...
  if (OUTPUT_RELAXED_MODEL) then
    ...
    if (NSPEC_CRUST_MANTLE > 0) then
      ...
      ! muv_relaxed
      call h5_write_dataset_collect_hyperslab_in_group('muv_relaxed', muvstore_crust_mantle*scaleval1, (/0, 0, 0, sum(offset_nspec_cm(0:myrank-1))/), if_col)
      ! muh_relaxed
      call h5_write_dataset_collect_hyperslab_in_group('muh_relaxed', muhstore_crust_mantle*scaleval1, (/0, 0, 0, sum(offset_nspec_cm(0:myrank-1))/), if_col)
      ! kappav_relaxed
      call h5_write_dataset_collect_hyperslab_in_group('kappav_relaxed', kappavstore_crust_mantle*scaleval1, (/0, 0, 0, sum(offset_nspec_cm(0:myrank-1))/), if_col)
      ! kappah_relaxed
      call h5_write_dataset_collect_hyperslab_in_group('kappah_relaxed', kappahstore_crust_mantle*scaleval1, (/0, 0, 0, sum(offset_nspec_cm(0:myrank-1))/), if_col)
